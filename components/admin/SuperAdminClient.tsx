  ...
  return (
    <div className="p-6 fade-in">
      <AdminPageHeader
        ...
        description="Manage admin access and module permissions for all users"
      />
...
function AccessControlTab() {
  ...
  const [confirm, setConfirm] = useState<{ user: UserRow; grant: boolean } | null>(null)
  ...
  const loginName = (u: UserRow) => `${u.first_name} ${u.last_name}`.trim() || u.login_username

  async function handleToggle(user: UserRow, grant: boolean) {
    setBusy(user.loginId)
    setConfirm(null)
    const res  = await fetch('/api/admin/super-admin', {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ userId: user.userId, grantAdmin: grant, source: 'user' }),
    })
    const data = await res.json()
    if (data.success) {
      showToast(grant ? `Admin access granted to ${loginName(user)}` : `Admin access revoked from ${loginName(user)}`)
      await load()
    } else {
  ...
  const filtered = users.filter(u => {
    const role = u.role ?? 0
    if (filter === 'admin'  && role < 1)  return false
    if (filter === 'client' && role >= 1) return false
    return !q
      || loginName(u).toLowerCase().includes(q)
      || (u.login_username || '').toLowerCase().includes(q)
      || (u.user_email || '').toLowerCase().includes(q)
      || (u.user_name || '').toLowerCase().includes(q)
  })

  const totalPages = Math.ceil(filtered.length / ACCESS_PER_PAGE)
  const safePage   = Math.min(page, Math.max(1, totalPages))
  const paginated  = filtered.slice((safePage - 1) * ACCESS_PER_PAGE, safePage * ACCESS_PER_PAGE)

  // Reset to page 1 when filter/search changes
  useEffect(() => { setPage(1) }, [search, filter])

  const adminCount  = users.filter(u => (u.role ?? 0) >= 1).length
  const clientCount = users.filter(u => (u.role ?? 0) === 0).length
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Login Users', value: users.length,  icon: '👥', color: '#14254A' },
          { label: 'Admin Access',      value: adminCount,    icon: '🛡️', color: '#0078D4' },
          { label: 'Client Users',      value: clientCount,   icon: '👤', color: '#6b7280' },
        ].map(s => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-3 border-b border-gray-100">
          <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
            {(['all', 'admin', 'client'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all ${filter === f ? 'bg-white shadow text-[#14254A]' : 'text-gray-500 hover:text-gray-700'}`}>
          ...
          <input autoComplete="off" value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="border border-gray-200 rounded-xl px-3 py-1.5 text-xs w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-[#14254A]/20" />
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: '#14254A' }}>
                {['#', 'Login User', 'Username', 'Client Company', 'Role', 'Status', 'Admin Access'].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                const role     = u.role ?? 0
                const roleInfo = ROLE_LABEL[role] ?? ROLE_LABEL[0]
                const isAdmin  = role >= 1
                const isBusy   = busy === u.loginId
                return (
                  <tr key={u.loginId} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3 text-xs text-gray-400 font-mono">{(safePage - 1) * ACCESS_PER_PAGE + idx + 1}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                          style={{ background: isAdmin ? '#0078D4' : '#14254A' }}>
                          {loginName(u).charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-medium text-gray-800 text-sm">{loginName(u)}</p>
                    ...
                    <td className="px-4 py-3 text-xs text-gray-600">{u.user_name}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setConfirm({ user: u, grant: !isAdmin })} disabled={isBusy}
                        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all disabled:opacity-50 ${isAdmin ? 'bg-red-50 text-red-600 border border-red-200 hover:bg-red-100' : 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100'}`}>
                        {isBusy ? <span className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" /> : isAdmin ? '✕ Revoke Admin' : '+ Grant Admin'}
                      </button>
                    </td>
                  </tr>
                )
        ...
        <Pagination total={filtered.length} page={safePage} perPage={ACCESS_PER_PAGE} onChange={setPage} />
      </div>
      ...
      {confirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl mb-4 ${confirm.grant ? 'bg-blue-50' : 'bg-red-50'}`}>
              {confirm.grant ? '🛡️' : '⚠️'}
            </div>
            <h3 className="font-bold text-[#14254A] text-base mb-1">{confirm.grant ? 'Grant Admin Access' : 'Revoke Admin Access'}</h3>
            <p className="text-sm text-gray-500 mb-5">
              {confirm.grant
                ? `${loginName(confirm.user)} (${confirm.user.user_name}) will gain access to the admin portal.`
                : `${loginName(confirm.user)} (${confirm.user.user_name}) will lose admin portal access immediately.`}
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={() => handleToggle(confirm.user, confirm.grant)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold text-white hover:opacity-90 ${confirm.grant ? 'bg-blue-600' : 'bg-red-500'}`}>
                {confirm.grant ? 'Grant Access' : 'Revoke Access'}
              </button>
            </div>
          </div>
        </div>
