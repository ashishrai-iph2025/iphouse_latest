...
CREATE TABLE IF NOT EXISTS `dcp_user_login` (
  ...
  `twofa_secret`   VARCHAR(64)  DEFAULT NULL,
  `is_active`      TINYINT      NOT NULL DEFAULT 1 COMMENT 'may this account sign in — read by every auth path',
  `deleted`        TINYINT      NOT NULL DEFAULT 0 COMMENT 'row is not a live assignment: account deleted, or this company removed. deleted=1 IMPLIES is_active=0',
